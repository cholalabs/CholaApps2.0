...
class JPagination extends JObject
{
	...
	public function getLimitBox()
	{
		...
		// Build the select list.
		if ($app->isAdmin()) {
			$html = JHtml::_('select.genericlist',  $limits, $this->prefix . 'limit', 'class="inputbox" size="1" onchange="Chola.submitform();"', 'value', 'text', $selected);
		}
		else {
	...
	protected function _item_active(&$item)
	{
		...
		{
			if ($item->base > 0) {
				return "<a title=\"".$item->text."\" onclick=\"document.adminForm.." . $this->prefix . "limitstart.value=".$item->base."; Chola.submitform();return false;\">".$item->text."</a>";
			}
			else {
				return "<a title=\"".$item->text."\" onclick=\"document.adminForm.." . $this->prefix . "limitstart.value=0; Chola.submitform();return false;\">".$item->text."</a>";
			}
		}
		else {
