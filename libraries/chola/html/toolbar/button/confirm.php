...
class JButtonConfirm extends JButton
{
	...
	protected function _getCommand($msg, $name, $task, $list)
	{
		...
		if ($list) {
			$cmd = "javascript:if (document.adminForm.boxchecked.value==0){alert('$message');}else{if (confirm('$msg')){Chola.submitbutton('$task');}}";
		} else {
			$cmd = "javascript:if (confirm('$msg')){Chola.submitbutton('$task');}";
		}
