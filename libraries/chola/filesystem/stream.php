...
jimport('chola.filesystem.helper');
jimport('chola.utilities.utility');

/**
 * Cholapps Stream Interface
 *
 * The Cholapps stream interface is designed to handle files as streams
...
class JStream extends JObject
{
	...
	function open($filename, $mode='r', $use_include_path=false, $context=null, $use_prefix=false, $relative=false, $detectprocessingmode=false)
	{
		...
		if (isset($url['scheme'])) {
			// if we're dealing with a Cholapps stream, load it
			if (JFilesystemHelper::isCholaStream($url['scheme'])) {
