...
class JHelp
{
	...
	static function createURL($ref, $useComponent = false, $override = null, $component = null)
	{
		...
		$search = array(
			...
			'{major}',			// Chola major version number
			'{minor}',			// Chola minor version number
			'{maintenance}'		// Chola maintenance version number
		);
	...
	static function createSiteList($pathToXml, $selected = null)
	{
		...
		if (!$xml) {
			$option['text'] = 'English (GB) help.chola.org';
			$option['value'] = 'http://help.chola.org';
			$list[] = $option;
		}
		else {
			$option = array ();
