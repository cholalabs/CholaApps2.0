...
jimport('chola.filesystem.archive');
jimport('chola.filesystem.path');
jimport('chola.base.adapter');

/**
 * Chola base installer class
 *
...
class JInstaller extends JAdapter
{
	...
	function discover()
	{
		...
		{
			// Cholapps 1.5 installation adapter legacy support
	...
	public function parseSQLFiles($element)
	{
		...
		foreach ($element->children() as $file)
		{
			...
			if ($fCharset == $dbCharset && $fDriver == $dbDriver)
			{
				...
				jimport('chola.installer.helper');
	...
	public function parseFiles($element, $cid=0, $oldFiles=null, $oldMD5=null)
	{
		...
		// Get the client info
		jimport('chola.application.helper');
	...
	public function findManifest()
	{
		...
		if (!empty($xmlfiles))
		{
			...
			{
				// Is it a valid chola installation manifest file?
			...
			JError::raiseWarning(1, JText::_('JLIB_INSTALLER_ERROR_NOTFINDCHOLAXMLSETUPFILE'));
			return false;
		}
		else
		{
	...
	/**
	 * Is the xml file a valid Chola installation manifest file
	 *
	 * @access	private
