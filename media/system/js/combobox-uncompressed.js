...
*/
Chola.combobox = {};
Chola.combobox.transform = function(el, options)
{
	// Add the editable option to the select.
	var o = new Element('option', {'class':'custom'}).set('text', Chola.JText._('ComboBoxInitString', 'type custom...'));
	o.inject(el, 'top');
	...
	el.addEvent('keypress', function(e){
		...
		if (this.options.selectedIndex == 0)
		{
			...
			{
				if (customString == Chola.JText._('ComboBoxInitString', 'type custom...')) {
					customString = '';
				}
			}
			...
			if (e.key == 'backspace')
			{
				...
				if (customString == '') {
					customString = Chola.JText._('ComboBoxInitString', 'type custom...');
				}
...
// Load the combobox behavior into the Chola namespace when the document is ready.
window.addEvent('domready', function(){
	$$('select.combobox').each(function(el){
		Chola.combobox.transform(el);
	});
});
