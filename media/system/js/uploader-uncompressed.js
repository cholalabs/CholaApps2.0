...
var FancyUpload2 = new Class({
	...
	updateOverall: function() {
		this.overallTitle.set('html', Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_PROGRESS_OVERALL', 'Overall Progress').substitute({
			total: Swiff.Uploader.formatUnit(this.size, 'b')
		}));
		if (!this.size) {
			this.currentTitle.set('html', Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_TITLE', 'Current Title'));
...
FancyUpload2.File = new Class({
	...
	render: function() {
		if (this.invalid) {
			if (this.validationError) {
				var msg = Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_'+this.validationError, this.validationError);
		...
		this.element = new Element('li', {'class': 'file'}).adopt(
			...
			new Element('a', {
				'class': 'file-remove',
				href: '#',
				html: Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_REMOVE', 'Remove'),
				title: Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_REMOVE_TITLE', 'Remove Title'),
				events: {
					click: function() {
						this.remove();
						return false;
			...
			new Element('span', {'class': 'file-name', 'html': Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_FILENAME', 'Filename').substitute(this)}),
			this.info
	...
	onStart: function() {
		...
		this.base.currentTitle.set('html', Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_FILE', 'Current File').substitute(this));
	},

	onProgress: function() {
		this.base.overallProgress.start(this.base.percentLoaded);
		this.base.currentText.set('html', Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_PROGRESS', 'Current Progress').substitute({
	...
	onComplete: function() {
		...
		this.base.currentText.set('html', Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_UPLOAD_COMPLETED', 'Upload Completed'));
	...
	onError: function() {
		...
		var error = Chola.JText._('JLIB_HTML_BEHAVIOR_UPLOADER_FILE_ERROR', 'File Error').substitute(this);
