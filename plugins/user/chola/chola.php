 ...
 * @copyright	Copyright (C) 2005 - 2009 Cholalabs Software LLP. All rights reserved.
 ...
 * @subpackage	User.chola
 * @since		1.5
 */
class plgUserChola extends JPlugin
{
	/**
	...
	public function onUserAfterSave($user, $isnew, $success, $msg)
	{
		...
		if ($isnew) {
			...
			if ($app->isAdmin()) {
				...
				// Load user_chola plugin language (not done automatically).
				$lang = JFactory::getLanguage();
				$lang->load('plg_user_chola', JPATH_ADMINISTRATOR);

				// Compute the mail subject.
				$emailSubject = JText::sprintf(
					'PLG_USER_CHOLA_NEW_USER_EMAIL_SUBJECT',
					$user['name'],
					$config->get('sitename')
				);
				...
				$emailBody = JText::sprintf(
					'PLG_USER_CHOLA_NEW_USER_EMAIL_BODY',
					$user['name'],
					$config->get('sitename'),
					JUri::root(),
					$user['username'],
	...
	public function onUserLogin($user, $options = array())
	{
		...
		// Update the user related fields for the Chola sessions table.
		$db->setQuery(
			'UPDATE `#__session`' .
