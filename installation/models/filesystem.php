...
/**
 * Filesystem configuration model for the Chola Core Installer.
 *
 * @package		Chola.Installation
 * @since		1.6
 */
class JInstallationModelFilesystem extends JModel
{
	...
	function detectFtpRoot($options)
	{
		...
		// Check to see if Chola is installed at the FTP current working directory.
		$paths = array();
		...
		// Check all possible paths for the real Chola installation by comparing version files.
		$rootPath = false;
		$checkValue = file_get_contents(JPATH_LIBRARIES.DS.'chola'.DS.'version.php');
		foreach($paths as $tmp)
		{
			$filePath = rtrim($tmp, '/').'/libraries/chola/version.php';
			$buffer = null;
	...
	function verifyFtpSettings($options)
	{
		...
		$buffer = null;
		if ($ftp->read($root.'/libraries/chola/version.php', $buffer) === false) {
			$ftp->quit();
