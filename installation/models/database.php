...
jimport('chola.application.component.model');
jimport('chola.filesystem.file');
jimport('chola.database.database');
require_once JPATH_INSTALLATION.'/helpers/database.php';

/**
 * Database configuration model for the Chola Core Installer.
 *
 * @package		Chola.Installation
 * @since		1.6
 */
class JInstallationModelDatabase extends JModel
{
	...
	function initialise($options)
	{
		...
		$jlang->load('chola', JPATH_ADMINISTRATOR, 'en-GB', true);
		// Load the selected language
		$jlang->load('chola', JPATH_ADMINISTRATOR, $options->language, true);
		...
		if (empty($options->db_created))
		{
			...
			if ($utfSupport) {
				$schema = 'sql/'.(($type == 'mysqli') ? 'mysql' : $type).'/chola.sql';
			} else {
				$schema = 'sql/'.(($type == 'mysqli') ? 'mysql' : $type).'/chola_backward.sql';
			}
			...
			// Load the localise.sql for translating the data in chola.sql/chola_backwards.sql
			...
			// Handle default backend language setting. This feature is available for localized versions of Chola 1.5.
