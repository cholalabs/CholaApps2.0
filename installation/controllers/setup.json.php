...
/**
 * Setup controller for the Chola Core Installer.
 * - JSON Protocol -
 *
 * @package		Chola.Installation
 * @since		1.6
...
class JInstallationControllerSetup extends JController
{
	...
	function detectFtpRoot()
	{
		...
		// Attempt to detect the Chola root from the ftp account.
	...
	function removeFolder()
	{
		...
		if ($useFTP == true) {
			...
			if ($return)
			{
				$file = JPath::clean($options->ftp_root.'/chola.xml');
				if (file_exists($file))
				{
					$return = $ftp->delete($file);
				}
			...
			ob_start();
			$return = JFolder::delete($path) && (!file_exists(JPATH_ROOT.'/chola.xml') || JFile::delete(JPATH_ROOT.'/chola.xml'));
			ob_end_clean();
		}
...
/**
 * Chola Core Installation JSON Response Class
 *
 * @package		Chola.Installation
 * @since		1.6
 */
