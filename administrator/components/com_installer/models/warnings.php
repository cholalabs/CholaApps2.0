...
jimport('chola.application.component.modellist');
...
class InstallerModelWarnings extends JModelList
{
	...
	function getItems()
	{
		...
		if (!$tmp_path) {
			$messages[] = Array('message'=>JText::_('COM_INSTALLER_MSG_WARNINGS_CHOLATMPNOTSET'), 'description'=>JText::_('COM_INSTALLER_MSG_WARNINGS_CHOLATMPNOTSETDESC'));
		} else {
			if (!is_writeable($tmp_path)) {
				$messages[] = Array('message'=>JText::_('COM_INSTALLER_MSG_WARNINGS_CHOLATMPNOTWRITEABLE'), 'description'=>JText::sprintf('COM_INSTALLER_MSG_WARNINGS_CHOLATMPNOTWRITEABLEDESC', $tmp_path));
			}
		}
